/********************************************************************\
  ...
  $Id: midas.h 3576 2007-02-06 08:19:26Z ritt $
...
#if defined(OS_LINUX) || defined(OS_OSF1) || defined(OS_ULTRIX) ||             \
    defined(OS_FREEBSD) || defined(OS_SOLARIS) || defined(OS_IRIX) ||          \
    defined(OS_DARWIN)
#define OS_UNIX
#endif
...
#define GET_FARM                                                               \
  (1 << 2) /**< distribute events over several                                 \
               clients (farming)                  */
...
#define RO_TRANSITIONS                                                         \
  (RO_BOR | RO_EOR | RO_PAUSE | RO_RESUME) /**< At all transitions */
...
#define MLOG                                                                   \
  MT_LOG, __FILE__, __LINE__ /**< info message which is only logged */
#define MTALK MT_TALK, __FILE__, __LINE__ /**< info message for speech system  \
                                             */
#define MCALL                                                                  \
  MT_CALL, __FILE__, __LINE__ /**< info message for telephone call */
...
#define CMD_GET_LAST                                                           \
  CMD_GET_FIRST + 1 /* update this if you add new commands ! */
...
#define CMD_GET_DIRECT_LAST                                                    \
  CMD_GET_DIRECT + 5 /* update this if you add new commands ! */

#define CMD_ENABLE_COMMAND                                                     \
  (1 << 14) /* these two commands can be used to enable/disable */
#define CMD_DISABLE_COMMAND                                                    \
  (1 << 15) /* one of the other commands                        */
...
#define DF_PRIO_DEVICE                                                         \
  (1 << 2) /**< get demand values from device instead of ODB */
...
typedef struct eqpmnt {
  ...
  INT (*cd)
  (INT cmd, PEQUIPMENT); /**< Class driver routine                      */
...
typedef struct {
  ...
  INT (*analyzer)
  (EVENT_HEADER *, void *); /**< Pointer to user analyzer routine  */
...
INT EXPRT cm_query_transition(int *transition, int *run_number,
                              int *trans_time);
INT EXPRT cm_register_deferred_transition(INT transition,
                                          BOOL (*func)(INT, BOOL));
...
INT EXPRT cm_set_client_info(HNDLE hDB, HNDLE *hKeyClient, char *host_name,
                             char *client_name, INT computer_id, char *password,
...
INT EXPRT cm_get_watchdog_info(HNDLE hDB, char *client_name, DWORD *timeout,
                               DWORD *last);
...
INT EXPRT cm_set_msg_print(INT system_mask, INT user_mask,
                           int (*func)(const char *));
...
/*---- buffer manager ----*/
INT EXPRT bm_open_buffer(char *buffer_name, INT buffer_size,
                         INT *buffer_handle);
...
INT EXPRT bm_compose_event(EVENT_HEADER *event_header, short int event_id,
                           short int trigger_mask, DWORD size, DWORD serial);
...
INT EXPRT bm_add_event_request(INT buffer_handle, short int event_id,
                               short int trigger_mask, INT sampling_type,
                               void (*func)(HNDLE, HNDLE, EVENT_HEADER *,
                                            void *),
                               INT request_id);
INT EXPRT bm_delete_request(INT request_id);
INT EXPRT bm_send_event(INT buffer_handle, void *event, INT buf_size,
                        INT async_flag);
INT EXPRT bm_receive_event(INT buffer_handle, void *destination, INT *buf_size,
                           INT async_flag);
...
INT EXPRT db_open_database(char *database_name, INT database_size, HNDLE *hdb,
                           char *client_name);
...
INT EXPRT db_create_key(HNDLE hdb, HNDLE key_handle, char *key_name,
                        DWORD type);
INT EXPRT db_create_link(HNDLE hdb, HNDLE key_handle, char *link_name,
                         char *destination);
INT EXPRT db_set_value(HNDLE hdb, HNDLE hKeyRoot, char *key_name, void *data,
                       INT size, INT num_values, DWORD type);
INT EXPRT db_get_value(HNDLE hdb, HNDLE hKeyRoot, char *key_name, void *data,
...
INT EXPRT db_scan_tree(HNDLE hDB, HNDLE hKey, int level,
                       INT (*callback)(HNDLE, HNDLE, KEY *, INT, void *),
                       void *info);
INT EXPRT db_scan_tree_link(HNDLE hDB, HNDLE hKey, int level,
                            void (*callback)(HNDLE, HNDLE, KEY *, INT, void *),
                            void *info);
INT EXPRT db_get_path(HNDLE hDB, HNDLE hKey, char *path, INT buf_size);
INT EXPRT db_delete_key(HNDLE database_handle, HNDLE key_handle,
                        BOOL follow_links);
INT EXPRT db_enum_key(HNDLE hdb, HNDLE key_handle, INT index,
                      HNDLE *subkey_handle);
INT EXPRT db_enum_link(HNDLE hdb, HNDLE key_handle, INT index,
                       HNDLE *subkey_handle);
...
INT EXPRT db_get_key_info(HNDLE hDB, HNDLE hKey, char *name, INT name_size,
...
INT EXPRT db_get_data(HNDLE hdb, HNDLE key_handle, void *data, INT *buf_size,
                      DWORD type);
INT EXPRT db_get_data1(HNDLE hDB, HNDLE hKey, void *data, INT *buf_size,
                       DWORD type, INT *num_values);
INT EXPRT db_get_data_index(HNDLE hDB, HNDLE hKey, void *data, INT *buf_size,
...
INT EXPRT db_set_data_index(HNDLE hDB, HNDLE hKey, void *data, INT size,
                            INT index, DWORD type);
INT EXPRT db_set_data_index2(HNDLE hDB, HNDLE hKey, void *data, INT size,
                             INT index, DWORD type, BOOL bNotify);
INT EXPRT db_set_num_values(HNDLE hDB, HNDLE hKey, INT num_values);
INT EXPRT db_merge_data(HNDLE hDB, HNDLE hKeyRoot, char *name, void *data,
...
INT EXPRT db_check_record(HNDLE hDB, HNDLE hKey, char *key_name, char *rec_str,
                          BOOL correct);
INT EXPRT db_open_record(HNDLE hdb, HNDLE hkey, void *ptr, INT rec_size,
                         WORD access, void (*dispatcher)(INT, INT, void *),
                         void *info);
INT EXPRT db_close_record(HNDLE hdb, HNDLE hkey);
INT EXPRT db_get_record(HNDLE hdb, HNDLE hKey, void *data, INT *buf_size,
                        INT align);
INT EXPRT db_get_record_size(HNDLE hdb, HNDLE hKey, INT align, INT *buf_size);
INT EXPRT db_set_record(HNDLE hdb, HNDLE hKey, void *data, INT buf_size,
                        INT align);
...
INT EXPRT db_copy(HNDLE hDB, HNDLE hKey, char *buffer, INT *buffer_size,
                  char *path);
...
INT EXPRT db_sprintf(char *string, void *data, INT data_size, INT index,
                     DWORD type);
INT EXPRT db_sprintfh(char *string, void *data, INT data_size, INT index,
                      DWORD type);
INT EXPRT db_sscanf(char *string, void *data, INT *data_size, INT index,
                    DWORD type);
...
INT EXPRT rpc_client_connect(char *host_name, INT midas_port, char *client_name,
                             HNDLE *hConnection);
...
void EXPRT rpc_convert_single(void *data, INT tid, INT flags,
                              INT convert_flags);
...
INT EXPRT hs_enum_events(DWORD ltime, char *event_name, DWORD *name_size,
...
INT EXPRT hs_get_var(DWORD ltime, DWORD event_id, char *var_name, DWORD *type,
                     INT *n_data);
...
INT EXPRT hs_read(DWORD event_id, DWORD start_time, DWORD end_time,
                  ...
                  DWORD *time_buffer, DWORD *tbsize, void *data_buffer,
                  DWORD *dbsize, DWORD *type, DWORD *n);
...
TH1X EXPRT *h1_book(const char *name, const char *title, int bins, double min,
                    double max) {
  TH1X *hist;
...
template <typename TH1X>
TH1X EXPRT *h1_book(const char *name, const char *title, int bins,
                    double edges[]) {
  TH1X *hist;
...
TH2X EXPRT *h2_book(const char *name, const char *title, int xbins, double xmin,
...
template <typename TH2X>
TH2X EXPRT *h2_book(const char *name, const char *title, int xbins, double xmin,
                    double xmax, int ybins, double yedges[]) {
  TH2X *hist;
...
template <typename TH2X>
TH2X EXPRT *h2_book(const char *name, const char *title, int xbins,
                    double xedges[], int ybins, double ymin, double ymax) {
  TH2X *hist;
...
template <typename TH2X>
TH2X EXPRT *h2_book(const char *name, const char *title, int xbins,
                    double xedges[], int ybins, double yedges[]) {
  TH2X *hist;
...
#define H2_BOOK(n, t, xb, xmin, xmax, yb, ymin, ymax)                          \
