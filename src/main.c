...
INT main_event(EVENT_HEADER * pheader, void *pevent)
{
   ...
   Int_t reftimes[10]; 
   Double_t tof=0,toftdc1=0,toftdc2=0,toftdc3=0,toftdc4=0,toftdc5=0,toftdc6=0,toftdc7=0;
   Double_t tofC = 0;
...
//   t_pad1=pad1;
   ...
   for(int i = 0; i < ntdc; i++) {
	...
	hTDC2DModule[tdcmodule]->Fill(ref_time,channel); 		
      ...
      switch(channel){
		...
		case TOF_TDC_CHAN: if(t_tof==0) {
			
						toftdc1= ((double)ref_time); 
						 tof=((double)ref_time)+TOFoffset; 
						 //std::cout << ref_time << " " << ((double)ref_time) << " " << TOFoffset << " "  << tof << std::endl;
						 t_tof=tof;
   ...
   //std::cout << "process Event " << X1hits_dt<< std::endl; 
   if(X1hits_dt>=globals.min_x_wires  &&  X1hits_dt<globals.max_x_wires+1){
    	//std::cout << "X1hits_dt>=globals.min_x_wires  &&  X1hits_dt<globals.max_x_wires+1" << std::endl;
     ...
     if((globals.misswires+addwiregap)>(wrangeX1-X1hits_dt)){
	  ...
	  //   std::cout << "(globals.misswires+addwiregap)>(wrangeX1-X1hits_dt)" << std::endl;
