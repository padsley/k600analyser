...
SiliconData *W1SiliconSort(float *ADC_import, int ntdc, int *TDC_channel_import, float *TDC_value_import)
{
  ...
  for(int k=0;k<mTDC->GetSize();k++)
      ...
      for(int i=0;i<ADCsize;i++)
	{
	  ...
	  if(W1ADCTDCChannelTestFront(i,mTDC->GetChannel(k)) && ADC_import[i]>0)
	    {
	      ...
	      for(int l=0;l<mTDC->GetSize();l++)
		{
		  for(int j=64;j<ADCsize;j++)//Loop over the channels looking for backs
		      ...
		      if(ADC_import[j]>0)
			{
			  ...
			  if(W1FrontBackTest(i,j,energyi,energyj,si) && W1ADCTDCChannelTestBack(j,mTDC->GetChannel(l)))
			    {
...
bool W1ADCTDCChannelTestFront(int ADCChannel, int TDCChannel)
{
  bool result = false;
  for(int i=0;i<NumberOfW1;i++)
    {
...
bool W1ADCTDCChannelTestBack(int ADCChannel, int TDCChannel)
{
  bool result = false;
  for(int i=0;i<NumberOfW1;i++)
    {
