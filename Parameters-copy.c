...
void ParameterInit()
{
  printf("\n ParameterInit\n");
  masses = new double[4];
  ReadConfiguration();//printf("L113\n");
  PulseLimitsInit();//printf("L114\n");
  ADCInit();//printf("L115\n");
  QDCInit();//printf("L116\n");
  PrintParameters();
...
void CalibrationParametersInitNonLinear(int channel, int nparams)
{
  ADCCalibrationParameters[channel] = new double[nparams];
}
...
void ReadCalibrationParameters(std::string CalibFile)
{
  ...
  else
  {
	...
	else
	{
	  ...
	  printf("channel: %d\n",channel);
	  CalibInput >> LineBuffer;
	  nparams = atoi(LineBuffer.c_str());
	  printf("nparams: %d\n",nparams);
	  //	  ADCCalibrationParameters[channel] = new double[nparams];
	  CalibrationParametersInitNonLinear(channel,nparams);
	  for(int i=0;i<nparams;i++)
	    {
	      CalibInput >> LineBuffer;
	      double CurrentParameter = atof(LineBuffer.c_str());
	      ADCCalibrationParameters[channel][i] = CurrentParameter;
	    }

	  printf("Channel: %d\t",channel);
	  for(int i=0;i<nparams-1;i++)printf("Parameter %d: %f\t",i,ADCCalibrationParameters[channel][i]);
	  printf("Parameter %d: %f\n",nparams-1,ADCCalibrationParameters[channel][nparams-1]);
	}
      }
    }
  }
...
/*-------------------------------------------------*/
void SetADCChannelCalibration(int channel, int nparams, double* params)
{
  if(channel<ADCsize)
  {
    for(int i=0;i<nparams;i++)ADCCalibrationParameters[channel][i] = params[i];
  }
}
...
/*-------------------------------------------------*/
void TDCInit()//not really initialising the TDC modules - more the TDC offsets
{
  printf("TDCInit\n");
