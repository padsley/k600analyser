...
void MMMSiliconSort(float *ADC_import, int ntdc, int *TDC_channel_import, float *TDC_value_import, SiliconData *si)
{
  ...
  //printf("MMMSiliconSort L43 test\n");
  ...
  {				    // note however, you are looking at ALL TDC values, so most will be for the wirechambers
    //printf("total nr of TDC hits, including drift chamber: %i, over which this loop goes  ",mTDC.GetSize());	  
    if(MMMTDCFrontTest(mTDC.GetChannel(k)))      // make sure the TDC channel is for a ring/p-side
    {
	//printf("Step 1: TDC channel determined to be for an MMM ring \n");    

	//===============================================================================
	// RN 26 May 2026
	// with PR226 we cannot test for the N sides since they were not instrumented. 
	// And also we cannot assign detNum based on combination of P side and N side
	// And we cannot use indice l
	//================================================================================
	//for(int l=0;l<mTDC.GetSize();l++)//Loop over all of the TDC values *again* but in this case, looking for N sides
	//{
	...
	//	  { 
		...
		int DetNum = MMMTDCIdentifyDetector(mTDC.GetChannel(k));   // assign detector nr based on ring TDC channel info
		//printf("  Step 2: Detetor number established as %i \n",DetNum);    
		if(DetNum>0)	// Ensure it is a good detector number
		{	
		    // First establish i as the ADC channel for the associated ring nr. We dont work with ring nr, but ADC channel
		    int i = MMMADCChannelLimits[DetNum-1][0] + (mTDC.GetChannel(k) - MMMTDCChannelLimits[DetNum-1][0]);  
		    //printf("    Step 3: ADC chan for the MMM ring that was hit is  %i \n",i);

		    //Remember that above we loop over TDC channels. 
		    //And by now we have said that for the relevant event, for which we should have 1 TDC channel for a ring, 
		    //and 2 ADC channels, one for a ring and one for a sector, we already established the
		    //TDC channel, and from that determined that it is a valid TDC channel for a ring, we got the detector nr
		    //as well as which should be the ADC channel for the ring. 
		    //
		    //Now we test that the P-side ADC channel and TDC channel indeed belong to the same detector and their
		    //associated ADC values are at least nonzero.
		    if(MMMADCTDCChannelTestPSide(i,mTDC.GetChannel(k)) && ADC_import[i]>0)   
		    {
		    	//printf("      Step 4: We established the TDC and ADC hit belong to the same det and channel \n");
			// what we need to do now is to assign an ADC channel to the sector that saw the particle too.
			// The problem we have is that all ADC channels are always read out. 
			// The best we can do is to first find the 8 sectors associated with the specific DetNum
			// and then go through them and hardcode a signal theshold. So that if the ADC value is above
			// that threshold then we know this is the sector involved.
			// I choose to use 200 as my first test
	
			//int j = MMMADCChannelLimits[DetNum-1][2] + (mTDC.GetChannel(l) - MMMTDCChannelLimits[DetNum-1][2]);
			// I need a way to assign a value to j that is the sector nr, since I cannot use the TDC to help here!

			int j=-1;
			for(int t=MMMADCChannelLimits[DetNum-1][2];t<MMMADCChannelLimits[DetNum-1][3]+1;t++)
			{
			        if(ADC_import[t]>200){   
					j=t;
		    			//printf("        Step 5: Now we establish the ADC channel nr of the sector:   %i \n",j);
				//}
			//}
			...
			//{
					...
					//if(MMMFrontBackTest(i,j,energyi,energyj,si) && 0.5*(energyi+energyj)>400)
					if(MMMFrontBackTest(i,j,energyi,energyj,si))
					{
		    				//printf("          Step 6: Final hurdle: FrontBack test and energy frontback check \n");
...
int MMMTDCIdentifyDetector(int TDCFrontChannel)
{
  int result = -1;
  for(int i=0;i<NumberOfMMM;i++)
    {
      //if(TDCFrontChannel>=MMMTDCChannelLimits[i][0] && TDCFrontChannel<=MMMTDCChannelLimits[i][1] && TDCBackChannel>=MMMTDCChannelLimits[i][2] && TDCBackChannel<=MMMTDCChannelLimits[i][3])
      // For PR226 we did not have the n side/segments/ohmic side connected to the TDCs!
      if(TDCFrontChannel>=MMMTDCChannelLimits[i][0] && TDCFrontChannel<=MMMTDCChannelLimits[i][1])
	{
	  result = i+1;
	}
    }
