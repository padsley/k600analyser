...
extern std::vector<std::vector<double>> tdcOffsets_runDependent;
...
const double sigma = 0.1;//MeV - silicon energy resolution - used for the front-back energy cut condition
                            ...
                            if(MMMADCTDCChannelTestPSide(i,mTDC.GetChannel(k)) && ADC_import[i]>0)
                            {
                                ...
                                int j = MMMADCChannelLimits[DetNum-1][2] + (mTDC.GetChannel(l) - MMMTDCChannelLimits[DetNum-1][2]);
                                {
                                    ...
                                    {//printf("test\n");
                                        ...
                                        //if(MMMFrontBackTest(i,j,energyi,energyj,si) && MMMADCTDCChannelTestNSide(j,mTDC.GetChannel(l)) && 0.5*(energyi+energyj)>0.15)
                                        if(MMMFrontBackTest(i,j,energyi,energyj,si) && MMMADCTDCChannelTestNSide(j,mTDC.GetChannel(l)) && 0.5*(energyi+energyj)>0.35)
                                        {
                                            ...
                                            if(energyi>=energyj)
                                            {
                                                si->SetEnergyHighest(energyi);
                                            }
                                            else
                                            {
                                                si->SetEnergyHighest(energyj);
                                            }
                                            ...
                                            //std::cout << "TDCOffsets[mTDC.GetChannel(k)]: " << TDCOffsets[mTDC.GetChannel(k)] << std::endl;
                                            
                                            //------------------------------------------------
                                            //si->SetOffsetTime_Front(mTDC.GetValue(k) + TDCOffsets[mTDC.GetChannel(k)] + 200.0);
                                            //si->SetOffsetTime_Back(mTDC.GetValue(l) + TDCOffsets[mTDC.GetChannel(l)]);
                                            
                                            //------------------------------------------------
                                            double tdcOffset_Front = 0.0;
                                            double tdcOffset_Back = 0.0;
                                            
                                            if(RunNumber>=479 && RunNumber<=505)
                                            {
                                                tdcOffset_Front = tdcOffsets_runDependent[0][mTDC.GetChannel(k)];
                                                tdcOffset_Back = tdcOffsets_runDependent[0][mTDC.GetChannel(l)];

                                            }
                                            else if(RunNumber>=1016 && RunNumber<=1262)
                                            {
                                                tdcOffset_Front = tdcOffsets_runDependent[1][mTDC.GetChannel(k)];
                                                tdcOffset_Back = tdcOffsets_runDependent[1][mTDC.GetChannel(l)];
                                            }
                                            
                                            si->SetOffsetTime_Front(mTDC.GetValue(k) + tdcOffset_Front);
                                            si->SetOffsetTime_Back(mTDC.GetValue(l) + tdcOffset_Back);
...
bool MMMFrontBackTest(int FrontChannel, int BackChannel, double FrontEnergy, double BackEnergy, SiliconData *si)
{
    ...
    for(int i=0;i<NumberOfMMM;i++)
    {
        if(FrontChannel>=MMMADCChannelLimits[i][0] && FrontChannel<=MMMADCChannelLimits[i][1] && BackChannel>=MMMADCChannelLimits[i][2] && BackChannel<=MMMADCChannelLimits[i][3])//If the two hits are part of the front and back of the same detector, then consider that the event might be good - make sure that hit 'i' in the main loop in the sort is always the front. Then we don't get any double-counting
        {
            ...
            bool passEnergyCorrelationTest = false;
            
            
            if(std::abs(diff)<3*sigma)
            {
                passEnergyCorrelationTest = true;
            }
            
            
            /*
            //      For CAKE calibration since the energy calibration is yet to be determined.
            if(std::abs(diff)<30*sigma)
            {
                passEnergyCorrelationTest = true;
            }
            */
            
            if(passEnergyCorrelationTest)
            {
