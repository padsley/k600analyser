...
bool SiliconData::TestEvent()
{
    ...
    if(SiliconTimeOffset_Front.size() != hits){printf("Mismatched size: Offset Time\n"); result = false;}
    if(SiliconTimeOffset_Back.size() != hits){printf("Mismatched size: Offset Time\n"); result = false;}
...
void SiliconData::ClearEvent()
{
    ...
    SiliconTimeOffset_Front.clear();
    SiliconTimeOffset_Back.clear();
