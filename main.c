...
//TFile *ftxt=new TFile("test.txt?filetype=raw","RECREATE");    // RN test Oct 24
...
TH2F *hADCChannels_vs_ADCValues;
#endif
...
INT main_init(void)                    
{
   ...
   hWireVsTimeOffsetPID = new TH2F("hWireVsOffsetTimePID","Wire channel vs time (cablelenghts offsets incl) PID selected", 4500, 3000, 12000, 1000, 0, 1000);
   ...
   hADCChannels_vs_ADCValues = new TH2F("hADCChannels_vs_ADCValues", "hADCChannels_vs_ADCValues", 1000, 0., 4000, 32*ADCModules, 0., 32*ADCModules);  //Moved declaration of this histogram to not within the main loop -> i.e. don't declare it new each loop.
...
{
   //printf("sizeof(ADCchannel) = %i ; sizeof(ADC) = %i ;  ",sizeof(ADCchannel),sizeof(ADC));
   //printf("main.c L949\n");
   DWORD *ptdc;
   Int_t ntdc = 0;
...
//   if(PaddlePIDGatesFlagb==1){    //only keep analyzer busy with events that pass this rough cut
   ...
   for(int i = 0; i < ntdc; i++) {
	...
	if((offset_time >= gates.u1_driftt_low) && (offset_time <= gates.u1_driftt_hi)){            //drifttime gate 
      ...
      else if ((channelnew >= globals.x2_1st_wire_chan) && (channelnew < globals.x2_last_wire_chan) && (channelnew!=596) ) {   //only for X2 wireplane
...
/*   
   char evtinfo[100];				// RN test Oct 24   
   sprintf(evtinfo,"%d 1\n",qdc_counter1);      // RN test Oct 24
   ftxt->WriteBuffer(evtinfo,strlen(evtinfo));  // RN test Oct 24
*/
...
rn*/
   ...
   if(raw)
   {
     ...
     {
	//printf("sizeof(ADCchannel) = %i ; sizeof(ADC) = %i ;  ",sizeof(ADCchannel),sizeof(ADC));
        for(int j=0; j<(ADCModules*32); j++)
        {
		if(ADC[j]>200.0){
			hADCChannels_vs_TDCChannels->Fill(TDC_channel_export[i], ADCchannel[j]);
			hADCChannels_vs_ADCValues->Fill(ADC[j], ADCchannel[j]);
		}
        }
     }
   }
