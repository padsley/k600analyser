...
INT adc_init(void)
{
   ...
   //printf("ADC init routine: ADCModules = %d  \n",ADCModules);
...
{
   //printf("ADC evt routine: ADCModules = %d  \n",ADCModules);
   INT i, nwords;
   DWORD *padc;
   ...
   for(i=0; i<ADCsize;i++){
       ...
       //printf("sizeof(ADCchannel) = %d ; sizeof(ADC) = %d ;  sizeof(adc) = %d     ",sizeof(ADCchannel),sizeof(ADC),sizeof(adc));
