...
Specifically for the PR226 experiment, a 2026 re-attempt at analysis of the 12C data.

General comment:

There is not longer a file called CableLength.dat. The TDC offsets, valid for ALL TDC channels
that can include other detectors, are now in TDCoffsets.dat, or whatever its name is 
...
Specific comment on MMM things:
1. The variable "SilicinTimeOffsets" can be ignored. The TDC offsets are already implemented in tdc.c and the times appearing as
"SiliconTime" or "SiliconTimeFront" is already shifted and aligned.
